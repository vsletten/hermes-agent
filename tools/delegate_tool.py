...
def _resolve_delegation_credentials(cfg: dict, parent_agent) -> dict:
    ...
    If ``delegation.base_url`` is configured without ``delegation.provider``,
    ...
    (``effective_api_key = override_api_key or parent_api_key``). This
    preserves the direct-endpoint behavior for custom OpenAI-compatible
    servers that intentionally share the parent credential.

    If ``delegation.provider`` is configured, including together with a
    ``base_url``, the full credential bundle (base_url, api_key, api_mode,
    ...
    provider:model pair and avoids leaking the parent's key into a different
    provider endpoint.
